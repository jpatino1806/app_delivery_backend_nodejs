...
module.exports = {
    ...
    async findByCategory( req, res, next) {
        try {
            const id_category = req.params.id_category; //categoria seleccionada por el cliente
    ...
    async create(req, res, next){
        ...
        if (files.length === 0 ) {
            return res.status(501).json({
                message: 'Error al registrar el producto, no hay imagen',
                success: false
            });
        }else{
            try {
                ...
                console.log(`Error: ${error}`);
                return res.status(501).json({
                    message: `Error al registar el producto ${error}`,
                    success: false,
                    error: error
                })
            }        
